...
function appendEmitsProp(scriptAst, emitsArr) {
  ...
  if (hasEmits) {
    scriptAst.find("{ emits:$_$ }").each((fAst) => {
      ...
      const originEmits = match[0][0].value
        .replace("[", "")
        .replace("]", "")
        .replace(/\"/g, "'")
        .split(",");
    ...
    const emitsArrCode = emitList.join(",");
    scriptAst.replace(
      `export default {$$$}`,
      `export default {$$$, 
                emits:[${emitsArrCode}]
...
function addUtils($, extFunCode, outRootPath, outFilePath, fileName) {
  ...
  let outRootDirPath =
    outRootPath.indexOf(".") > -1 ? path.dirname(outRootPath) : outRootPath;
  ...
  const code = fs.existsSync(inputPath)
  ...
  let modulePath = withoutExt(
    path.relative(path.dirname(outFilePath), inputPath)
  );

  return modulePath === "gogocodeTransfer"
    ? "./gogocodeTransfer"
